CREATE TABLE "prod-dynamo"."ygg_dte_cedidas" WITH (
  format = 'parquet',
  external_location = 's3://staging.layer/DynamoDB/ygg_dte_cedidas/'
) AS
SELECT CAST (Item.cesionarioRut.N as int) as cesionario_rut,
  ...
  CAST(
    From_iso8601_timestamp(Item.fechaCesion.S) AS timestamp
  ) as fecha_cesion,
  CAST(
  ...
  CAST (Item.montoCesion.N as int) as monto_cesion,
  CAST (Item.montoTotal.N as int) as monto_total,
