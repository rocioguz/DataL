CREATE TABLE "prod-dynamo"."ygg_dte_recibidos" WITH (
  format = 'parquet',
  external_location = 's3://staging.layer/DynamoDB/ygg_dte_recibidos/'
) AS
SELECT CAST (Item.empRut.N as int) as emp_rut,
  ...
  CAST (Item.detalle.M.montos.M.afecto.N as bigint) as detalle_montos_afecto,
  CAST (Item.detalle.M.montos.M.exento.N as bigint) as detalle_montos_exento,
  ...
  CAST (Item.detalle.M.porVencer.BOOL as boolean) as detalle_por_vencer_total,
